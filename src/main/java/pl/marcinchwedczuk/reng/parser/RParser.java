...
public class RParser {
    ...
    private int curr;
    private int refGroupNum = 1;
    private HashMap<Integer, RAst> refGroup = new HashMap<Integer, RAst>();
    ...
    private RAst Gterm() {
        ...
        if (lookahead(0, RTokenType.AT_BEGINNING)) {
            ...
            if (lookahead(0, RTokenType.QMARK)) {
                consume(RTokenType.QMARK);
                if (lookahead(0, RTokenType.EQUAl)) {
                    consume(RTokenType.EQUAl);
                    RAst check = lookAheadCheck();
                    refGroup.put(refGroupNum, check);
                    refGroupNum++;
                    consume(RTokenType.RPAREN);
                    return RAst.posLookAhead(check);
                } else if(lookahead(0, RTokenType.EXCLAMATION)){
                    consume(RTokenType.EXCLAMATION);
                    RAst check = lookAheadCheck();
                    refGroup.put(refGroupNum, check);
                    refGroupNum++;
                    consume(RTokenType.RPAREN);
                    return RAst.negLookAhead(check);
                }
                else {
                    throw new RParseException(currToken().pos,
            ...
            RAst tmp = Gregex();
            consume(RTokenType.RPAREN);
            refGroup.put(refGroupNum, tmp);
            refGroupNum++;
            return tmp;
            }
        }
        else if(lookahead(0, RTokenType.BACKREF)){
            int group = getNumericValue(currToken().c);
            consume( RTokenType.BACKREF);
            return GbackReference(group);
        }
        else {
            throw new RParseException(currToken().pos,
                    "Unexpected token " + currToken().type + ".");
    ...
    public RAst lookAheadCheck() {
        List<RAst> alternatives = new ArrayList<>();

        alternatives.add(Gconcatenation());

        while (!lookahead(0, RTokenType.RPAREN)) {
            alternatives.add(Gconcatenation());
        }
   ...
   private RAst GbackReference(int refNum){

       if(refNum <= refGroupNum){
            return refGroup.get(refNum);
       }
       else {
           throw new RParseException(currToken().pos,
                   "Unexpected group number " + getNumericValue(currToken().c));
       }
    }
    public int getRefGroupNum(){
        return refGroupNum;
    }
